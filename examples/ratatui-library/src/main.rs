//! Library-mode example — `ono` is a regular `cargo` dependency; nothing is
//! vendored. Contrast with `examples/ratatui-demo/`, which runs the same
//! scenes from an ejected `src/ono/` tree.
...
use ono::theme::{Knobs, Palette, Theme};
...
fn render_dashboard(
    ...
    Dashboard::new(state, now, palette, knobs)
        .subtitle("ratatui library example")
